...
def draw_cat_plot():
    ...
    df_cat = pd.melt(
        df,
        id_vars=["cardio"],
        value_vars=["choles_norm", "gluc_norm", "smoke", "alco", "active", "overweight"],
    )
...
def draw_heat_map():
    ...
    lower_mask = np.tri(corr.shape[0], corr.shape[1], k=-1)
    ...
    sns.heatmap(corr, mask=upper_mask, annot=True, fmt=".1f", ax=ax)
